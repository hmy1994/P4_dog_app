...
def count(files):
    count_true = 0
    for i in range(100):
        if face_detector(files[i]) == True:
            count_true += 1
    return count_true
print('human_files_short: %d' % count(human_files_short)+'%')
print('dog_files_short: %d' % count(dog_files_short)+'%')
...
# 
# __回答:__  
# 如果是对于不明确的场景提出这样的要求当然是不合理的，比如说交通违法的人脸识别。这需要一些已经经过训练的算法然后进行再训练。
# 
...
def count_dog_detector(files):
    count_true = 0
    for i in range(100):
        if dog_detector(files[i]) == True:
            count_true += 1
    return count_true
print('human_files_short: %d' % count_dog_detector(human_files_short)+'%')
print('dog_files_short: %d' % count_dog_detector(dog_files_short)+'%')
...
# 原因是它包含了卷积层、池化层、全池化层  
# 卷积层的作用：  
# 起局部感知作用，类似于人脑识别图片的过程，先对于图中的每一个特征进行识别。  
#   
# 池化层的作用：  
# 用于特征降维，压缩数据和参数的数量，减小过拟合，同时提高模型的容错性。  
#   
# 全局池化的作用：  
# 降低维度，从3维降低到1维，为每个特征图输出1个响应操作。
...
# __回答:__   
# 第一步建立一个Sequential，  
# 第二步增加一个全局池化层，可以将输入的数组转变为向量，减少需要训练的参数  
# 第三步增加一个全连接层，参数是需要分类的数量  
# 使用迁移学习的好处是可以继承其他人经过大量训练和大量调试的架构和超参数  
# Xception实际上采用了类似于ResNet的网络结构，主体部分采用了模块化设计。
# Xception是Inception体系结构的扩展，它用深度可分的卷积替换标准的Inception模块。  
# 主要是我感觉它很先进。
...
def dog_or_human(img_path):
    ...
    if dog_detector(img_path)==1:
        ...
        print('Human,but very like this dog:')
        print(Xception_predict_breed(img_path))
    else:
        print("This is not a human or a dog!")
...
## 自由地使用所需的代码单元数吧
path=[]
path.append('../dog-project/images/3.jpg')
path.append('../dog-project/images/4.jpg')
path.append('../dog-project/images/cat.jpg')
path.append('../dog-project/images/dznq.jpg')
path.append('../dog-project/images/erha.jpg')
path.append('../dog-project/images/jpsf.jpg')
path.append('../dog-project/images/goda.jpg')
